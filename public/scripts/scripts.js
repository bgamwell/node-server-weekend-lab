$(document).ready(function() {
  ...
  $.ajax({
    ...
    return postID = $(this).data("id"); // reassigning global variable postID with this data id
  ...
  $('#delete-a-post').on('click', function(){
    ...
    $.ajax({
      url: 'http://localhost:3000/api/posts/' + postID, // there's no get request first, so how does ajax know what the id is?
      type: 'DELETE',
      success: function() {
        $("#edit-post-modal").modal("hide");
        window.location.reload();
